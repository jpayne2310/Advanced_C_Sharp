...
namespace Structure
{
    class myValidate
    {
        ...
        //check for maximum length of masked phone
        ...
        //check for valid Facebook Address
        public static bool IsFacebookValid(string temp)
        {
            bool result = true;
            if (temp.Length != 0)
            {
                //look for position of first period
                int atLocation = temp.IndexOf(".");
                //look for position of last period
                int periodLocation = temp.LastIndexOf(".");

                //check for 3 character before first period
                if (atLocation < 3)
                {
                    result = false;
                }

                //check for at least 8 characters between first period and last
                else if ((atLocation + 8) >= periodLocation)
                {
                    result = false;
                }

                //chechk for at least three character after last period
                else if (periodLocation >= (temp.Length - 3))
                {
                    result = false;
                }
            }
